...
- `metadata` (continuous and categotical matrix): 81 rows and 37 columns. Contains much of the metadata for each of the samples. For this case study, only variables used from this data is the `experiment` feature (denotes which study each sample is drawn from) and `inhib_inoc` feature (denotes the type of inhibition for that sample).
...
A visualisation of the tuning process can be seen in Figure 2. In reality, the `test.keepX` should be iterated on, but for the sake of this analysis, multiples of five were tested. The fairly equal standard distributions of each test shows the consistency of the model across multiple folds and repeats.
