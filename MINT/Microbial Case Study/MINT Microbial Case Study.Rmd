...
Using just the training data, the model is first tuned. Observing the error rates for each `keepX` on each component, the minimum is achieved by `keepX = 17` on the first component. As visualisation is desired, a minimum component count of 2 will be used. The next time that the error rate is minimised is `keepX = 32` on the second component. Hence, the optimal number of components is two and the optimal number of features is 17 and 32 respectively. 
...
<p style="text-align: center;">FIGURE 3: Feature loading values on the global, first (a) and second (b) component. Bars coloured based on the treatment class with the highest median on that variable.</p>

The correlation circle plot (Figure 4) shows that a majority of the selected features have strong associations (both positive and negative) with the first component. There is a smaller set of those which define the second component. Those on the negative side of the first component are more tightly clustered, suggesting features that define the ammonia class are more homogeneous and the placebo group has signals that are more disparate. 
...
<p style="text-align: center;">FIGURE 4: Correlation circle plot representing the OTUs selected by MINT sPLS-DA performed on the microbial data to examine the association of the selected OTUs on the first two components. A minimum correlation value of 0.5 is required to be depicted on these axes.</p>
...
<p style="text-align: center;">FIGURE 5: Clustered Image Map of the OTUs selected by MINT sPLS-DA on the microbial data using all three components. A hierarchical clustering based on the counts of the selected OTUs was used, with samples in rows coloured according to the inoculation method.</p>
...
<p style="text-align: center;">FIGURE 6: Relevance network of the selected features on the first component by MINT sPLS-DA performed on the microbial OTU data. Dotted, red lines are positive while solid, green lines are negative. Minimum correlation value of 0.7 required to be shown.</p>

### Testing the model on studies 3 and 4

After the model has been explored, it's performance as a classifier is to be assessed. Here, the `perf()` function is used to determine which distance metric will be most appropriate to use. Addition of the second component either has no impact on the model performance, or in the case of the mahalanobis distance, worsened performance. Based on Figure 7, classification error rate is minimised using `max.dist`.
...
<p style="text-align: center;">FIGURE 7: Classification error rate of the MINT sPLS-DA models, using up to two components for all three distance metrics.</p>
...
As can be seen below, the error rate and balanced error both decrease with the addition of the second component, though the model was already quite good at classifying these novel samples with just one component. This is unsurprising as looking at Figure 2, the first component separated the two classes entirely. 
...
Below, Figure 8 goes to show that sometimes the AUROC plot can show off some misleading results. For models using either one or both components the AUC is 1 despite the fact that the error rate wasn't 0. 

``` {r, fig.align = "center"}
auroc(ab.mint.splsda, roc.comp = 2, print = FALSE)
```

<p style="text-align: center;">FIGURE 8: ROC curve and AUC from the MINT sPLS-DA performed on the microbial OTU data using both components. Numerical output on right is the AUC values for the model.</p>
